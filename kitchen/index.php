...
async function load(){
 ...
 d.orders.forEach(o=>{
   ...
   if(o.status==='done'){
     ...
     el.innerHTML=`<summary><span><strong>#${esc(o.order_number)}</strong><small>${itemCount(o.items)} item${itemCount(o.items)===1?'':'s'} · ${time(o.updated_at||o.created_at)}</small></span><b>${money(o.total)}</b></summary><div class="done-details">${items}<div class="order-total">Total ${money(o.total)}</div></div>`;
   ...
   el.innerHTML=`<div class="order-top"><div class="order-num">#${esc(o.order_number)}</div><div class="order-time">${time(o.created_at)}</div></div>${items}<div class="order-total">${money(o.total)}</div><button class="btn btn-yellow" style="width:100%" onclick="setStatus(${Number(o.id)},'${next}')">${label}</button>`;target.appendChild(el);
 });
}
