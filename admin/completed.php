...
if(isset($_GET['export'])&&$_GET['export']==='csv'){
 ...
 $out=fopen('php://output','w');fputcsv($out,['Order #','Name','Special Requests','Completed','Drink','Milk','Qty','Unit Price','Line Total','Order Total']);
 foreach($orders as $o){$first=true;foreach($o['items'] as $i){fputcsv($out,[$o['order_number'],$o['customer_name'],$o['notes'] ?? '',$o['updated_at'],$i['drink_name'],$i['milk_type'],(int)$i['quantity'],number_format((float)$i['price'],2,'.',''),number_format((float)$i['price']*(int)$i['quantity'],2,'.',''),$first?number_format((float)$o['total'],2,'.',''):'']);$first=false;}}
 fclose($out);exit;
}
...
<div class="panel completed-table-wrap"><table class="table completed-table"><thead><tr><th class="select-col"><span class="sr-only">Select</span></th><th>Order</th><th>Name</th><th>Special Requests</th><th>Completed</th><th>Items / Details</th><th>Qty</th><th>Price</th><th>Total</th><th>Delete</th></tr></thead><tbody>
<?php if(!$orders):?><tr><td colspan="10">No completed orders yet.</td></tr><?php endif;?>
<?php foreach($orders as $o): $qty=0;foreach($o['items'] as $i)$qty+=(int)$i['quantity'];?><tr><td class="select-col"><input class="order-checkbox" type="checkbox" name="order_ids[]" value="<?=h($o['id'])?>" aria-label="Select order <?=h($o['order_number'])?>"></td><td><strong>#<?=h($o['order_number'])?></strong></td><td><strong><?=h($o['customer_name'] ?: '—')?></strong></td><td class="notes-cell"><?=!empty($o['notes'])?nl2br(h($o['notes'])):'—'?></td><td><?=h(date('M j, Y g:i A',strtotime($o['updated_at'])))?></td><td><?php foreach($o['items'] as $i):?><div class="history-item"><strong><?=h($i['quantity'])?>× <?=h($i['drink_name'])?></strong><?php if($i['milk_type']):?><span><?=h($i['milk_type'])?></span><?php endif;?> <small>@ $<?=number_format((float)$i['price'],2)?></small></div><?php endforeach;?></td><td><?=$qty?></td><td class="muted-cell"><?=count($o['items'])===1?'$'.number_format((float)$o['items'][0]['price'],2):'Mixed'?></td><td><strong>$<?=number_format((float)$o['total'],2)?></strong></td><td><button type="submit" class="btn btn-danger btn-delete-order" data-order-id="<?=h($o['id'])?>" data-order-number="<?=h($o['order_number'])?>">DELETE</button></td></tr><?php endforeach;?>
